...
def count_records(cur):
    ...
    count = cur.fetchone() [0] # The 0 referres to the very first index in the in tuple.
    return cur,count
...
def onDelete(self):
    ...
    with conn:
        cur = conn.cursor()
        # check count to ensure that this is not the last record in the datebase.... connot delete last
        # record or we will get an error
        cur.execute(""" SELECT COUNT(*) FROM tbl_phonebook""")
        count = cur.fetchone()[0]
        if count > 1:
            confirm = messagebox.askokcancel('Delete Confirmation', "All information associated with, {} \nwill be prmenantly deleted from the database.\n\nProceed with the deletion request?".format(var_select))
            if confirm:
                conn = sqlite3.connect('phonebook.db')
                with conn:
                    cursor = conn.cursor()
...
######                  onRefresh(self) # update the listbox of the changes
            ...
            confirm = messagebox.showerror("Last Record Error", "({}) is the last record in the datebase and connot be deleted at this time \n\nPlease add another record first before you can delete ({}).".format(var_select,var_select))
    conn.close()
...
def onDeleted(self):
    ...
    self.entry_phone.delete(0,END)
    self.entry_email.delete(0,END)
## onRefresh(self) # update the list box of the changes
    try:
...
def onClear(self):
    #claer the text in these textboxes
    self.entry_fname.delete(0,END)
    self.entry_lname.delete(0,END)
    self.entry_phone.delete(0,END)
    self.entry_email.delete(0,END)
...
def onRefresh(self):
    # Populate the lstList1, coinciding with the database                  
